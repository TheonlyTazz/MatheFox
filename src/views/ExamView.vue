...
<template>
  <main class="mx-auto max-w-4xl px-4 py-6 sm:py-10">
    <div v-if="!finished && current" class="rounded-[2rem] bg-white p-5 shadow-sm sm:p-8"><div class="mb-6 flex items-start justify-between gap-3"><div><p class="font-bold text-violet-600">Probearbeit · {{ active + 1 }}/{{ examExercises.length }}</p><h1 class="text-2xl font-black text-stone-800">{{ test.title }}</h1></div><div class="flex items-center gap-2 rounded-xl bg-violet-50 px-3 py-2 font-black text-violet-700"><Clock :size="18" /> {{ timeLabel }}</div></div><ExerciseCard :key="current.id" :exercise="current" @solved="solved" /><div class="mt-5 flex justify-between"><button class="rounded-xl px-4 py-2 font-bold text-stone-500 hover:bg-stone-100" @click="active = Math.max(0, active - 1)">Zurück</button><button v-if="active < examExercises.length - 1" class="rounded-xl bg-violet-600 px-5 py-2 font-bold text-white hover:bg-violet-700" @click="active++">Nächste</button><button v-else class="rounded-xl bg-emerald-600 px-5 py-2 font-bold text-white hover:bg-emerald-700" @click="finish">Abgeben</button></div></div>
