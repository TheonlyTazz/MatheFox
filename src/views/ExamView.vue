...
const finish = (): void => {
  ...
  certificate.value = {
    ...
    result: { label: 'Ergebnis', value: `${score.value} von ${examExercises.length} richtig` },
    issuedAt: new Date(),
  }
  finished.value = true
...
const openCertificate = (): void => {
  if (!certificate.value) throw new Error('Für die Prüfung ist noch kein Zertifikat verfügbar.')
...
<template>
  <main class="mx-auto max-w-4xl px-4 py-6 sm:py-10 md:px-6 md:py-12">
    <div v-if="!finished && current" class="rounded-[1.5rem] bg-white p-4 shadow-sm sm:rounded-[2rem] sm:p-8 md:p-10"><div class="mb-6 flex flex-col gap-3 min-[420px]:flex-row min-[420px]:items-start min-[420px]:justify-between md:mb-8"><div><p class="font-bold text-violet-600">Probearbeit · {{ active + 1 }}/{{ examExercises.length }}</p><h1 class="text-2xl font-black leading-tight text-stone-800 md:text-3xl">{{ test.title }}</h1></div><div class="flex w-fit items-center gap-2 rounded-xl bg-violet-50 px-3 py-2 font-black text-violet-700"><Clock :size="18" /> {{ timeLabel }}</div></div><ExerciseCard :key="current.id" :exercise="current" @solved="solved" /><div class="mt-5 grid grid-cols-1 gap-2 min-[420px]:flex min-[420px]:justify-between md:mt-7"><button :disabled="active === 0" class="min-h-11 rounded-xl px-4 py-2 font-bold text-stone-500 hover:bg-stone-100 disabled:opacity-40" @click="active = Math.max(0, active - 1)">Zurück</button><button v-if="active < examExercises.length - 1" class="min-h-11 rounded-xl bg-violet-600 px-5 py-2 font-bold text-white hover:bg-violet-700" @click="active++">Nächste</button><button v-else class="min-h-11 rounded-xl bg-emerald-600 px-5 py-2 font-bold text-white hover:bg-emerald-700" @click="finish">Abgeben</button></div></div>
    <div v-else class="rounded-[2rem] bg-white p-8 text-center shadow-sm"><Trophy class="mx-auto text-amber-500" :size="58" /><h1 class="mt-4 text-3xl font-black text-stone-800">Geschafft!</h1><p class="mt-2 text-stone-500">Du hast {{ score }} von {{ examExercises.length }} Aufgaben richtig gelöst.</p><div class="mx-auto mt-6 max-w-xs rounded-2xl bg-amber-50 p-5"><strong class="text-4xl text-amber-600">{{ Math.round((score / examExercises.length) * 100) }}%</strong><p class="mt-1 text-sm text-amber-800">Dein Mathearbeit-Ergebnis</p></div><button type="button" class="mt-6 flex mx-auto min-h-11 items-center gap-2 rounded-xl bg-amber-500 px-5 py-3 font-bold text-white hover:bg-amber-600" @click="openCertificate">Zertifikat ansehen</button><button class="mt-3 flex mx-auto items-center gap-2 rounded-xl bg-orange-500 px-5 py-3 font-bold text-white" @click="emit('home')"><RotateCcw :size="18" /> Zur Startseite</button></div>
