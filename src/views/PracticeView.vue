...
<template>
  <main class="mx-auto max-w-6xl px-4 py-6 sm:py-10">
    ...
    <div :id="`panel-${selected}`" class="mx-auto max-w-3xl" role="tabpanel" :aria-labelledby="`tab-${selected}`"><ExerciseCard :key="current.id" :exercise="current" :show-next="submittedExerciseIds.has(current.id) && index < session.length - 1" @solved="solved" @next="nextExercise" /><div class="mt-5 flex items-center text-sm text-stone-500"><span class="flex items-center gap-2"><BookOpen :size="17" /> Aufgabe {{ index + 1 }} von {{ session.length }}</span></div></div>
