...
<template>
  <main class="mx-auto max-w-6xl px-4 py-6 sm:py-10">
    ...
    <div :id="`panel-${selected}`" class="mx-auto max-w-3xl" role="tabpanel" :aria-labelledby="`tab-${selected}`"><ExerciseCard :key="current.id" :exercise="current" @solved="solved" /><div class="mt-5 flex flex-col gap-3 text-sm text-stone-500 min-[420px]:flex-row min-[420px]:items-center min-[420px]:justify-between"><span class="flex items-center gap-2"><BookOpen :size="17" /> Aufgabe {{ (index % currentExercises.length) + 1 }} von {{ currentExercises.length }}</span><button class="flex min-h-11 items-center justify-center gap-2 font-bold text-orange-600 min-[420px]:justify-end" @click="index++">Nächste Aufgabe <ClipboardPenLine :size="17" /></button></div></div>
