...
<template>
  <div v-if="open" class="fixed inset-0 z-40 grid place-items-center bg-stone-950/30 p-4 backdrop-blur-sm" @click.self="emit('close')">
    <section class="max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-y-auto rounded-3xl bg-white p-5 shadow-2xl sm:p-7" role="dialog" aria-modal="true" aria-labelledby="topics-title">
      ...
      <label class="mt-5 flex min-h-14 cursor-pointer items-center justify-between gap-3 rounded-2xl bg-violet-50 px-4 py-3 font-bold text-violet-900"><span>🔊 Fragen automatisch vorlesen</span><input type="checkbox" class="h-6 w-6 accent-violet-600" :checked="profile.autoReadQuestions" @change="profile.setAutoReadQuestions(($event.target as HTMLInputElement).checked)" /></label>
